...
// getKEPRPath — корень файлов раздела «КЭПР».
func getKEPRPath() string {
	path := os.Getenv("KEPR_DOCS_PATH")
	if path == "" {
		path = "data/kepr"
	}
	if !filepath.IsAbs(path) {
		...
		if _, err := os.Stat(path); os.IsNotExist(err) {
			path = filepath.Join(cwd, "..", "data", "kepr")
		}
	}
	return path
}

// ListKEPR — список папок и файлов (публичное чтение).
func ListKEPR(c *gin.Context) {
	basePath := getKEPRPath()
...
func AdminCreateKEPRFolder(c *gin.Context) {
	var req struct {
...
func AdminUploadKEPRFile(c *gin.Context) {
...
func AdminDeleteKEPRItem(c *gin.Context) {
