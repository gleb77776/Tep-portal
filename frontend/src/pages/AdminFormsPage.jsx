...
function buildFileUrl(path, name) {
  ...
  return backendUrl(`/forms/files/${parts.join('/')}`);
}
...
function AdminFormsPage({ onOpenDocument }) {
  const token = localStorage.getItem(ADMIN_TOKEN_KEY);
  ...
  const refreshList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(backendUrl(`/api/v1/forms/list?path=${encodeURIComponent(path)}`));
  ...
  const persistAll = async () => {
    ...
    try {
      ...
      for (const rel of sortedDeletes) {
        const res = await fetch(adminApiUrl(`/forms-item?path=${encodeURIComponent(rel)}`), {
          method: 'DELETE',
          headers: auth,
        });
      ...
      for (const { path: p, name } of foldersSorted) {
        const res = await fetch(adminApiUrl('/forms-folder'), {
          method: 'POST',
      ...
      for (const { path: p, file } of uploadsSnapshot) {
        ...
        const res = await fetch(adminApiUrl('/forms-upload'), {
          method: 'POST',
  ...
  if (access && access.role !== 'administrator' && access.role !== 'hr') {
  ...
  return (
    <div className="admin-news-page smk-page">
      <div className="admin-news-header">
        <h1 className="admin-news-title">Бланки — редактирование</h1>
      ...
      <p className="admin-form-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Как в разделах СМК и КЭПР: очередь папок и файлов, сохранение на сервер — кнопкой «Сохранить». Документы на сайте по
        адресу /forms.
      </p>
