...
/** Читает тело ответа один раз: JSON или текст (для отладки 401/502/HTML). */
...
function buildFileUrl(path, name) {
  ...
  return backendUrl(`/smk/files/${parts.join('/')}`);
}
...
function AdminSMKPage({ onOpenDocument }) {
  const token = localStorage.getItem(ADMIN_TOKEN_KEY);
  ...
  const refreshList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(backendUrl(`/api/v1/smk/list?path=${encodeURIComponent(path)}`));
  ...
  const persistAll = async () => {
    ...
    try {
      ...
      for (const rel of sortedDeletes) {
        const res = await fetch(adminApiUrl(`/smk-item?path=${encodeURIComponent(rel)}`), {
          method: 'DELETE',
          headers: auth,
        });
      ...
      for (const { path: p, name } of foldersSorted) {
        const res = await fetch(adminApiUrl('/smk-folder'), {
          method: 'POST',
        ...
        if (!res.ok) {
          throw new Error(
            `Папка «${name}» (${p}): ${await readFetchError(res)}`
          );
        }
      }
      setPendingFolders([]);
      ...
      for (const { path: p, file } of uploadsSnapshot) {
        ...
        const res = await fetch(adminApiUrl('/smk-upload'), {
          method: 'POST',
  ...
  return (
    <div className="admin-news-page smk-page">
      <div className="admin-news-header">
        <h1 className="admin-news-title">СМК — редактирование раздела</h1>
      ...
      <p className="admin-form-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Создание папок, загрузка и удаления попадают в очередь. На сайте изменения появятся после нажатия «Сохранить» в
        шапке.
      </p>
