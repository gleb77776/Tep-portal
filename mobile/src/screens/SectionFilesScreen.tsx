...
export type SectionFilesConfig = {
  fetchList: (relPath: string) => Promise<OtListResponse>;
  buildFileUrl: (relPath: string, fileName: string) => string;
  pageTitle: string;
...
export default function SectionFilesScreen({ config }: Props) {
  ...
  const applyList = useCallback((data: OtListResponse) => {
    if (data.error) {
      setError(data.error);
      setFolders([]);
      setFiles([]);
    ...
    setFolders([...(data.folders ?? [])].sort(byName));
    setFiles([...(data.files ?? [])].sort(byName));
  }, []);

  /** Те же данные, что на сайте (/ohs, /forms): GET …/list + файлы с …/files — обновление при фокусе экрана и смене папки. */
  useEffect(() => {
    if (!isFocused) return;
    let cancelled = false;
    setLoading(true);
  ...
  }, [path, applyList, config.fetchList, isFocused]);

  async function onRefresh() {
    setRefreshing(true);
    try {
      const data = await config.fetchList(path);
      applyList(data);
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Ошибка загрузки');
      setFolders([]);
  ...
  function openFileUrl(name: string) {
    const url = config.buildFileUrl(path, name);
      ...
      {!loading && !error ? (
        <View style={styles.list}>
          ...
          {folders.length === 0 && files.length === 0 ? (
            <Text style={[styles.empty, { color: colors.textMuted }]}>{config.emptyMessage}</Text>
          ) : null}
        </View>
      ) : null}
    </ScrollView>
