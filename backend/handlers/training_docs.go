...
// getTrainingPath — корень файлов раздела «Записи с программ обучения».
func getTrainingPath() string {
	path := os.Getenv("TRAINING_DOCS_PATH")
	if path == "" {
		path = "data/training"
	}
	if !filepath.IsAbs(path) {
		...
		if _, err := os.Stat(path); os.IsNotExist(err) {
			path = filepath.Join(cwd, "..", "data", "training")
		}
	}
	return path
}

// ListTraining — список папок и файлов (публичное чтение).
func ListTraining(c *gin.Context) {
	basePath := getTrainingPath()
...
func AdminCreateTrainingFolder(c *gin.Context) {
	var req struct {
...
func AdminUploadTrainingFile(c *gin.Context) {
...
func AdminDeleteTrainingItem(c *gin.Context) {
