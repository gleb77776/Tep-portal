...
import { canEditOT, canonicalAdminRole } from '../utils/adminRoleAccess';
...
function buildFileUrl(path, name) {
  ...
  return backendUrl(`/ot/files/${parts.join('/')}`);
}
...
function AdminOTPage({ onOpenDocument }) {
  const token = localStorage.getItem(ADMIN_TOKEN_KEY);
  ...
  const refreshList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(backendUrl(`/api/v1/ot/list?path=${encodeURIComponent(path)}`));
  ...
  const persistAll = async () => {
    ...
    try {
      ...
      for (const rel of sortedDeletes) {
        const res = await fetch(adminApiUrl(`/ot-item?path=${encodeURIComponent(rel)}`), {
          method: 'DELETE',
          headers: auth,
        });
      ...
      for (const { path: p, name } of foldersSorted) {
        const res = await fetch(adminApiUrl('/ot-folder'), {
          method: 'POST',
      ...
      for (const { path: p, file } of uploadsSnapshot) {
        ...
        const res = await fetch(adminApiUrl('/ot-upload'), {
          method: 'POST',
  ...
  if (access && !canEditOT(access)) {
    return <Navigate to="/admin" replace />;
  }

  const cr = access ? canonicalAdminRole(access) : '';
  const backFromOT =
    access && ['administrator', 'documentation', 'hr'].includes(cr)
      ? '/admin/sections'
      : '/admin';
  const backFromOTLabel = backFromOT === '/admin/sections' ? '← К разделам' : '← Назад';

  return (
    <div className="admin-news-page smk-page">
      <div className="admin-news-header">
        <h1 className="admin-news-title">Охрана труда, ГО и ЧС — редактирование</h1>
        <div className="admin-news-actions">
          <Link to={backFromOT} className="admin-btn admin-btn-secondary" style={{ textDecoration: 'none' }}>
            {backFromOTLabel}
          </Link>
      ...
      <p className="admin-form-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Как в разделе СМК: очередь папок и файлов, сохранение на сервер — кнопкой «Сохранить». Документы на сайте по
        адресу /ohs.
      </p>
