...
function AdminTrainingPage({ onOpenDocument }) {
  const token = localStorage.getItem(ADMIN_TOKEN_KEY);
  ...
  const refreshList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(backendUrl(`/api/v1/training/list?path=${encodeURIComponent(path)}`));
  ...
  const persistAll = async () => {
    ...
    try {
      ...
      for (const rel of sortedDeletes) {
        const res = await fetch(adminApiUrl(`/training-item?path=${encodeURIComponent(rel)}`), {
          method: 'DELETE',
          headers: auth,
        });
      ...
      for (const { path: p, name } of foldersSorted) {
        const res = await fetch(adminApiUrl('/training-folder'), {
          method: 'POST',
      ...
      for (const { path: p, file } of uploadsSnapshot) {
        ...
        const res = await fetch(adminApiUrl('/training-upload'), {
          method: 'POST',
  ...
  const openPreview = (fileRow) => {
    ...
    if (fileRow.__pending && fileRow.objectUrl) {
      ...
      url = buildFileUrl(path, fileName);
    }
    if (onOpenDocument) {
      onOpenDocument({
        name: fileName,
  ...
  const canPreview = (name) => {
    ...
    return PREVIEW_EXT.includes(ext) || VIDEO_EXT.includes(ext);
  };

  if (!isAuthed) {
    return <Navigate to="/admin/login" replace />;
  }
  if (access && access.role !== 'administrator') {
  ...
  return (
    <div className="admin-news-page smk-page">
      <div className="admin-news-header">
        <h1 className="admin-news-title">Записи с программ обучения — редактирование</h1>
      ...
      <p className="admin-form-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Очередь папок и файлов, сохранение — «Сохранить». На сайте: документы и видео (mp4, webm и др.) — просмотр и
        скачивание. Раздел /training.
      </p>
        ...
        <div className="smk-content">
          <div className="smk-list">
            ...
            {displayFiles.map((f) => {
              ...
              const fileUrl = f.__pending && f.objectUrl ? f.objectUrl : buildFileUrl(path, fileName);
              const showPreview = canPreview(fileName);
              const extLower = (fileName || '').split('.').pop()?.toLowerCase() || '';
              const isVid = VIDEO_EXT.includes(extLower);
              return (
                <div key={`${rel}-file`} className="smk-item smk-item--file">
                  <span className="smk-item__icon smk-item__icon--file">{isVid ? '🎬' : '📄'}</span>
                  ...
                  <div className="smk-item__actions">
                    {showPreview && onOpenDocument && (
                      <button
                        ...
                        title={isVid ? 'Воспроизвести' : 'Предпросмотр'}
                      >
                        {isVid ? '▶ Смотреть' : '👁 Просмотр'}
                      </button>
                    )}
                    <a
                      href={fileUrl}
            ...
            {displayFolders.length === 0 && displayFiles.length === 0 && (
              <p className="smk-page__empty">Папка пуста. Создайте папку или загрузите документ / видео.</p>
            )}
          </div>
        </div>
      )}
      ...
      {showUpload && (
        <div className="smk-modal-overlay" onClick={() => !saving && setShowUpload(false)}>
          <div className="smk-modal" onClick={(e) => e.stopPropagation()}>
            <div className="smk-modal-header">
              <h3>Загрузить файл или видео</h3>
