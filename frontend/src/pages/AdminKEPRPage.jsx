...
function AdminKEPRPage({ onOpenDocument }) {
  const token = localStorage.getItem(ADMIN_TOKEN_KEY);
  ...
  const refreshList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(backendUrl(`/api/v1/kepr/list?path=${encodeURIComponent(path)}`));
  ...
  const persistAll = async () => {
    ...
    try {
      ...
      for (const rel of sortedDeletes) {
        const res = await fetch(adminApiUrl(`/kepr-item?path=${encodeURIComponent(rel)}`), {
          method: 'DELETE',
          headers: auth,
        });
      ...
      for (const { path: p, name } of foldersSorted) {
        const res = await fetch(adminApiUrl('/kepr-folder'), {
          method: 'POST',
      ...
      for (const { path: p, file } of uploadsSnapshot) {
        ...
        const res = await fetch(adminApiUrl('/kepr-upload'), {
          method: 'POST',
  ...
  return (
    <div className="admin-news-page smk-page">
      <div className="admin-news-header">
        <h1 className="admin-news-title">КЭПР — редактирование</h1>
      ...
      <p className="admin-form-hint" style={{ marginTop: -8, marginBottom: 16 }}>
        Как в разделах СМК и ОТ: очередь папок и файлов, сохранение на сервер — кнопкой «Сохранить». Документы на сайте по
        адресу /kepr.
      </p>
