...
// getFormsPath — корень файлов раздела «Бланки».
func getFormsPath() string {
	path := os.Getenv("FORMS_DOCS_PATH")
	if path == "" {
		path = "data/forms"
	}
	if !filepath.IsAbs(path) {
		...
		if _, err := os.Stat(path); os.IsNotExist(err) {
			path = filepath.Join(cwd, "..", "data", "forms")
		}
	}
	return path
}

// ListForms — список папок и файлов (публичное чтение).
func ListForms(c *gin.Context) {
	basePath := getFormsPath()
...
func AdminCreateFormsFolder(c *gin.Context) {
	var req struct {
...
func AdminUploadFormsFile(c *gin.Context) {
...
func AdminDeleteFormsItem(c *gin.Context) {
